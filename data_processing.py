...
from Angle_Calculator import calculateAngles

#load data files and convert to dataframes
...
#converts all files to Data frames
def load_datasets():
...
#Adds dilation column to FXD dataframe
...
#giving colour as per dilation value
def dilation_color(FXD):
    avg = FXD['dilation'].mean()
    FXD['percentage_change'] = ((FXD['dilation'].sub(avg)).div(avg)).mul(100)
    FXD['color'] = 'black'
    FXD.loc[FXD['percentage_change'] > 3., 'color'] = 'lightblue'
    return FXD

#Find relative and absolute angles and add it to FXD dataframes
...
# create new column in FXD dataframe for hover property of Plot design.(Interactive)
def hover(FXD):
    hover_text = []
...
#Add Title to Data Visualization
fig.update_layout(
...
#Save and Open HTML
fig.write_html('TreevsGraphDV.html', auto_open=True)